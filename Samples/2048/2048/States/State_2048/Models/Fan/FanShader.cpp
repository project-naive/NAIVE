...
namespace State_2048 {
	namespace Shaders {
		GLuint Fan::CreateProgram(){
			std::ifstream ifs;
			ifs.open(vertex_shader_path);
			if (!ifs) {
				std::cerr << "Error opening Fan vertex shader file!\n" << std::endl;
				return 0;
			}
			...
			if (!ifs) {
				std::cerr << "Error opening Fan fragment shader file!\n" << std::endl;
			...
			if (link_result == GL_FALSE) {
				...
				std::cerr << "Error linking Fan Program!\n\n" << info_log.data() << '\n' << std::endl;
				glDeleteShader(VertexShader);
