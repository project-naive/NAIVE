...
namespace Engine {
	namespace Graphics {
			...
			typedef struct {
				...
				typedef struct {
					typedef struct {
						...
						//do not use unless initializing, 
						//since the lifetime is not guaranteed
				...
				Context_Creation_Info GLContext;
			} Window_Exended_Info;
		...
		class GLContext {
//			friend Managers::GLContext;
			friend ::GLContextManager;
			friend ::GLContextServer;
		protected:
			//Not constant because of window hints
			//GLContext creation switches the current context to initialize glew
			//No switching back to make things faster. The manager knows such property
			//And manages the interaction
			GLContext(WindowInfo* info=nullptr, GLContext* shared=nullptr);
			~GLContext();
			WindowInfo display;
