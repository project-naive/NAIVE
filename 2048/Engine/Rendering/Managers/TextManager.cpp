...
namespace Rendering {
	namespace Manager {
		...
		size_t Text::loadFace(const char* data, size_t size, size_t index) {
			size_t new_font_index = fonts.load_face((const FT_Byte*)data, size, index);
		...
		bool Rendering::Manager::Text::unloadFace(size_t texture_index) {
			if (!fonts.unload_face(font_index[texture_index])) {
				return false;
			}
		...
		void Text::genCharTexture(
			...
			FT_Face face = fonts.faces[font_index[texture_index]];
